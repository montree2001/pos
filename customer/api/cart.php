                ...
                success: function(response) {
                    try {
                        if (response && response.success) {
                            ...
                            throw new Error(response ? response.message : 'Invalid response');
                        }
                    } catch (e) {
                        console.error('Response processing error:', e);
                        showToast('error', 'เกิดข้อผิดพลาดในการประมวลผลข้อมูล');
                    }
            ...
            $.ajax({
                ...
                success: function(data, textStatus, xhr) {
                    try {
                        ...
                        showToast('error', 'เกิดข้อผิดพลาดในการประมวลผลข้อมูล กรุณาลองใหม่');
                ...
                error: function(xhr, status, error) {
                    ...
                    if (status === 'timeout') {
                        ...
                        errorMessage = 'ไม่พบไฟล์ API สำหรับสร้างออเดอร์';
                    } else if (xhr.status === 500) {
                        errorMessage = 'เกิดข้อผิดพลาดภายในเซิร์ฟเวอร์';
                    } else if (xhr.responseText) {
                        try {
